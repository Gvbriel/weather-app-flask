...
from flask import Flask, redirect, url_for, render_template, request, Response, flash, abort
...
def continentName(Weather):
    countryalpha = pc.country_name_to_country_alpha2(
        Weather.country, cn_name_format="default")
...
class Weather:
    ...
    def __init__(self, city, country):
        ...
        self.long = geolocator.geocode(
            self.city + ',' + self.country).longitude

        self.capitol = CountryInfo(self.country).capital()
        self.dailyForecaster = wthmgr.forecast_at_place(
            self.city + ', ' + self.country, '3h')
        self.oneCallForecast = wthmgr.one_call(
            self.lat, self.long).forecast_daily
        ...
        try:
            ...
            self.timezone = CountryInfo(str(self.country)).timezones()[
                0] + '/' + self.citySpace
        ...
        self.sunrise = self.sunrise.replace(
            tzinfo=pytz.utc).astimezone(self.zone)
    ...
    def longTermForecast(self):
        for weather in self.oneCallForecast:
            self.forecastTempMax.append(
                round(float(weather.temperature('celsius').get('max')), 1))
            self.forecastTempMin.append(
                round(float(weather.temperature('celsius').get('min')), 1))
...
citieslist = [Warsaw, Amsterdam, NewYork, Tokyo, Shanghai,
              BuenosAires, LosAngeles, Paris, Bangkok, Madrid, Manila, Moscow]
...
def search():
    if request.method == "POST":
        ...
        try:
            wthmgr.weather_at_place(str(cityInput) + ', ' + str(countryInput))

        except:
            return render_template('search.html', mes="Something went wrong, try again...")

        return redirect(url_for('city', city=cityInput, country=countryInput))
    else:
...
def city():
    ...
    plt.clf()
    # plt.xkcd()
    plt.plot(dates, tempMax, color='#1f77b4',
             marker='.', label="Highest temperature")
    plt.plot(dates, tempMin, color='#17becf',
             marker='.', label="Lowest temperature")
    ...
    for i, j in zip(dates, tempMin):
        axes.annotate(str(j), xy=(i, j), xytext=(-13, -15),
                      textcoords='offset points')

    for i, j in zip(dates, tempMax):
        axes.annotate(str(j), xy=(i, j), xytext=(-13, 10),
                      textcoords='offset points')
